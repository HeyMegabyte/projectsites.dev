...
describe('Dockerfile orchestrator wiring', () => {
  ...
  it('copies build_validators.ts so run-validators.mjs can import it', () => {
    expect(dockerfile).toMatch(
      /COPY\s+src\/services\/build_validators\.ts\s+\/home\/cuser\/build_validators\.ts/,
    );
  });

  it('copies run-validators.mjs into the container', () => {
    expect(dockerfile).toMatch(
      /COPY\s+scripts\/run-validators\.mjs\s+\/home\/cuser\/run-validators\.mjs/,
    );
  });

  it('syncs universal agents from ~/.agentskills/agents/ to ~/.claude/agents/', () => {
    expect(dockerfile).toMatch(
      /cp\s+\/home\/cuser\/\.agentskills\/agents\/\*\.md\s+\/home\/cuser\/\.claude\/agents\//,
    );
  });
  ...
  it('symlinks AGENTS.md + _router.md into /home/cuser for non-Claude tools', () => {
    expect(dockerfile).toMatch(
      /ln -sf\s+\/home\/cuser\/\.agentskills\/AGENTS\.md\s+\/home\/cuser\/AGENTS\.md/,
    );
    expect(dockerfile).toMatch(
      /ln -sf\s+\/home\/cuser\/\.agentskills\/_router\.md\s+\/home\/cuser\/_router\.md/,
    );
  });
