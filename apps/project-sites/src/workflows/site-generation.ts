...
function buildPrompt(params: SiteGenerationParams, hasSourceBrand = false): string {
  ...
  return [
    ...
    'No server-rendered routes. No DB calls from rendered pages. Forms POST to `/api/contact-form/${slug}` via the inlined widgets.js hijack pattern (`<script src="https://projectsites.dev/widgets.js" data-slug="${slug}" defer>`). widgets.js bundles form-hijack + audio-reactive hero + Konami dev console. All dynamic data is baked at build-time + 30s polling via inline JS. Never introduce SSR, API handlers, or third-party backends.',
    '',
