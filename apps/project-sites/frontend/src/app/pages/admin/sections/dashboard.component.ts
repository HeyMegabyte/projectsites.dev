          ...
          <div class="flex items-start gap-5 max-md:flex-col">
            ...
            <div class="flex-1 min-w-0 flex flex-col gap-2">
              ...
              <div class="flex items-center gap-1.5 text-sm text-text-secondary">
                <a [href]="state.getSiteUrl(site)" target="_blank" rel="noopener"
                   class="text-primary/70 font-mono text-xs hover:text-primary transition-colors no-underline">{{ site.slug }}.projectsites.dev</a>
                <button class="icon-btn-sm opacity-60 hover:opacity-100 cursor-pointer" (click)="state.copyUrl(site)" title="Copy URL">
              ...
              <div class="flex gap-2 flex-wrap mt-1 items-center">
                @if (site.status === 'published') {
                  <button class="btn-accent text-sm cursor-pointer" (click)="goToEditor()">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>
                    Edit with AI
                  </button>
                  <button class="btn-ghost text-sm cursor-pointer" (click)="state.visitSite(site)">
        ...
        <div class="grid grid-cols-2 gap-4 items-stretch max-lg:grid-cols-1">
          ...
          <div class="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-5 flex flex-col h-full">
            ...
            @if (recentLogs().length === 0) {
              <div class="text-[0.78rem] text-text-secondary/50 py-4 text-center flex-1 flex items-center justify-center">No activity yet</div>
            } @else {
          ...
          <div class="grid grid-cols-2 gap-3 h-full max-sm:grid-cols-1">
            <div class="section-link-card" (click)="goTo('/admin/domains')">
              <div class="flex flex-col gap-2 min-w-0 flex-1">
                ...
                <span class="text-[0.7rem] text-text-secondary">{{ domainCount() }} connected</span>
              </div>
            ...
            <div class="section-link-card" (click)="goTo('/admin/billing')">
              <div class="flex flex-col gap-2 min-w-0 flex-1">
                ...
                <span class="text-[0.7rem] text-text-secondary">{{ state.subscription()?.status === 'active' ? 'Pro plan active' : 'Free plan' }}</span>
              </div>
              <span class="text-[0.7rem] text-primary/50 self-end">Manage &rarr;</span>
            </div>
            ...
            <div class="section-link-card" (click)="goTo('/admin/analytics')">
              <div class="flex flex-col gap-2 min-w-0 flex-1">
                ...
                <span class="text-[0.7rem] text-text-secondary">Traffic and visitors</span>
              </div>
              <span class="text-[0.7rem] text-primary/50 self-end">Manage &rarr;</span>
            </div>
          </div>
...
export class AdminDashboardComponent implements OnInit {
  ...
  domainCount = (): number => this.state.domainSummary().total + 1;
